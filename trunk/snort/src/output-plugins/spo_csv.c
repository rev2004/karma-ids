        ...
        int numargs, Event *event)
{
	...
	{
        if(IPH_IS_VALID(p))
            fputs(inet_ntoa(GET_SRC_ADDR(p)), file);
	}
	else if(!strncasecmp("dst", type, 3))
	{
        if(IPH_IS_VALID(p))
            fputs(inet_ntoa(GET_DST_ADDR(p)), file); 
	}
