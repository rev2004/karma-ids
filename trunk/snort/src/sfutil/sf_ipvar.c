...
sfip_node_t *sfipnode_alloc(char *str, SFIP_RET *status) {
    ...
    if(!strncasecmp(str, "any", 3)) {
        ...
        if( !(ret->ip = sfip_alloc("0.0.0.0", NULL)) ) {
            if(status)
                *status = SFIP_FAILURE;
            free(ret);
            return NULL;
...
SFIP_RET sfvar_add_node(sfip_var_t *var, sfip_node_t *node, int negated) {
    ...
    if((node->flags & SFIP_ANY) || 
       (sfip_cmp(node->ip, (*head)->ip) == SFIP_LESSER)) {
        node->next = *head;
        *head = node;
        return SFIP_SUCCESS;
    }
    ...
    for(p = *head; p->next; p=p->next) {
        if(sfip_cmp(node->ip, p->next->ip) == SFIP_LESSER) {
            swp = p->next;
            p->next = node;
            node->next = swp;
...
SFIP_RET sfvar_compare(sfip_var_t *one, sfip_var_t *two) {
    ...
    for(idx1 = one->head; idx1; idx1 = idx1->next)
    {
        ...
        {
            if((sfip_cmp(idx1->ip, idx2->ip) == SFIP_EQUAL) && !usage[i])
            {
                match = 1;
                usage[i] = 1;
                break;
...
SFIP_RET sfvar_parse_iplist(vartable_t *table, sfip_var_t *var,
    ...
    while(*str) {
        ...
        if(*str == LIST_OPEN) {
            ...
            sfvar_add(var, tmp_var);

            /* Apply the negation */
            if(negation ^ neg_ip) {
                /* Check for a negated "any" */
                if(var->head && var->head->flags & SFIP_ANY) {
                    free(tok);
                    return SFIP_NOT_ANY;
                }

                /* Check if this is a negated, zero'ed IP (equivalent of a "!any") */
                if(!sfip_is_set(var->head->ip)) {
                    free(tok);
                    return SFIP_NOT_ANY;
                }
