...
void AlertFast(Packet *p, char *msg, void *arg, Event *event)
{
    ...
    if(p && IPH_IS_VALID(p))
    {
        ...
        fprintf(data->file, "{%s} ", protocol_names[GET_IPH_PROTO(p)]);

        if(p->frag_flag)
        {
            /* just print the straight IP header */
            fputs(inet_ntoa(GET_SRC_ADDR(p)), data->file);
            fwrite(" -> ", 4, 1, data->file);
#ifdef SUP_IP6
            fputs(sfip_ntoa(GET_DST_ADDR(p)), data->file);
#else
            fputs(inet_ntoa(GET_DST_ADDR(p)), data->file);
#endif
        }
        else
        {
    ...
    if(p && data->packet_flag)
    {
        ...
        if(IPH_IS_VALID(p))
            PrintIPPkt(data->file, GET_IPH_PROTO(p), p);
        else if(p->ah)
