...
rule_count_t tcpCnt={0,0,0,0,0},udpCnt={0,0,0,0,0},icmpCnt={0,0,0,0,0},ipCnt={0,0,0,0,0};
...
PortObject * ParsePortListTcpUdpPort( char * port_str )
{
    ...
    { 
      ...
      if( !portobject )
      {
          FatalError("***Src PortVar Lookup failed on '%s'\n",name);
      }
    ...
    {   
       ...
       if( !portobject )
       {
          ...
          FatalError("***Rule--PortVar Parse error: (pos=%d,error=%s)\n>>%s\n>>%*s\n",
 ...
 *
 * Arguments: rtn: RuleTreeNode.
 *
 * Returns: 1 if a conflict exists.  0 otherwise.
 *
 ***************************************************************************/
...
#define PREPROCESOR_RULES    
    ...
    if(CheckForIPListConflicts(proto_node.sip))
    {
    ...
    */
    ...
    /* Make sure the IP lists provided by the user are valid */
    if(CheckForIPListConflicts(proto_node.dip))
    {
...
int ProcessIP(char *addr, RuleTreeNode *rtn, int mode, int neg_list)
{
    ...
    {   
        int ret;
        if( !rtn->sip && !(rtn->sip = calloc(1, sizeof(sfip_var_t))) ) 
        {
    ...
    {
        int ret;
        if( !rtn->dip && !(rtn->dip = calloc(1, sizeof(sfip_var_t))) ) 
        {
            FatalError("%s(%d) => Failed to allocate memory for address: %s\n",
                file_name, file_line, addr);
        }
...
{
#ifdef SUP_IP6
     if( (sfip_cmp(one->ip, two->ip) != SFIP_EQUAL) ||
...
int PortVarDefine( char * name, char * s )
{
    ...
    if( strstr(s,"any") ) /* this allows 'any' or '[any]' */
    {
      if(strstr(s,"!"))
      {
    ...
    else
    {
      ...
      if(!po)
      {
         errstr = PortObjectParseError( &pop );
          FatalError("*** PortVar Parse error: (pos=%d,error=%s)\n>>%s\n>>%*s\n",
    ...
    if( rstat < 0 )
    {
        FatalError("***PortVarTableAdd failed with '%s', exiting ",po->name);
    }
    else if( rstat > 0 )
    {
...
int VarIsIpAddr(char *value) 
{
    ...
    if( isdigit(*value) &&
         !strchr(value+1, (int)':') &&
         ((tmp = strchr(value+1, (int)'/')) && isdigit( *(tmp+1) )) )
        return 1;

    /* IPv6 */
    if((tmp = strchr(value, (int)':'))) 
    {
        char *tmp2;

        if(!(tmp2 = strchr(tmp+1, (int)':'))) 
            return 0;
...
struct VarEntry *VarDefine(char *name, char *value)
{
    ...
    {
        sfip_var_t *var;
        if((var = sfvt_lookup_var(vartable, value))) 
        {
...
// value is never stored, and is never meant to be used.
    ...
    if(!(var = sfvt_lookup_var(vartable, name))) {
